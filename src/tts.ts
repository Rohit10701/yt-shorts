...
export class ElevenLabsVoice extends VoiceGen {
    ...
    static async generateVoice(gen: VideoGen, options: VoiceGenOptions, apiKey?: string) {
        if (!apiKey) {
            console.log("ElevenLabs API key required for voice generation. Set via '--elevenlabsAPIKey' option or define 'ELEVENLABS_API_KEY' environment variable.");
        ...
        console.log("Generating voice in ElevenLabs");
        return await new Promise<void>(async (resolve, reject) => {
            try {
                ...
                console.log("Error generating voice in ElevenLabs");
                reject(error);
            }
        });
